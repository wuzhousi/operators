...
cv::Mat ConvertMyArray2OpencvMat(const std::unique_ptr<MyArray> & ptr){
  ...
  if(channels==3){
    ...
    for(int j=0; j<cols; j++){
      for(int i=0; i<rows; i++){
        mat.at<cv::Vec3b>(i, j)[0] = static_cast<char>(array_ptr[0 * cols * rows + j * rows + i]);
        mat.at<cv::Vec3b>(i, j)[1] = static_cast<char>(array_ptr[1 * cols * rows + j * rows + i]);
        mat.at<cv::Vec3b>(i, j)[2] = static_cast<char>(array_ptr[2 * cols * rows + j * rows + i]);
      }
    }
    return mat;
  }
    ...
    for(int j=0; j<cols; j++){
      for(int i=0; i<rows; i++){
        mat.at<uchar>(i, j) = static_cast<char>(array_ptr[0 * cols * rows + j * rows + i]);
      }
    }
    return mat;
  }
